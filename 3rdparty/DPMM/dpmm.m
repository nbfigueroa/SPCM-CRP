...
%     params(1).alpha = 1 / wishrnd(1,1); %1 / wishrnd(1,1);
%     params(1).kappa = .1; %T / 1000; %a pseudo count on the mean
    params(1).kappa = T/1000; %T / 1000; %a pseudo count on the mean
    params(1).nu = 6; %a pseudo-count on the covariance
%     params(1).nu = dimension; %a pseudo-count on the covariance
    params(1).initmean = allmean;
%     params(1).initcov = allcov / 10;
    params(1).initcov = allcov ;
